...
{{ each message-field }}<dnl>
                .{{ helper message-field-name }} = {{ helper message-field-unpack-fn-name }}({{ helper message-field-name }}_blob),
{{ end }}<dnl>
            }
        }
    };
...
{{ helper field-getter-sig }}
{
    ...
    return (const {{ helper message-field-blob-type }} *)(&(self->{{ helper message-field-name }}));
}
{{ end }}<dnl>
{{ end }}<dnl>
